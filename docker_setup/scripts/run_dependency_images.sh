...
docker run -d --net host  --name redis_cont \
...
docker run -d --net host --name nats_cont nats:1.1.0 -p 4222
docker logs nats_cont &> $etaLogDir/nats_server.log

echo "4. Starting mosquitto container..."
docker run -d --net host --name mosquitto_cont \
...
docker run -d --net host --name postgres_cont \
