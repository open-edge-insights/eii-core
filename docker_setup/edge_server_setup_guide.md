...
master = true" > /etc/uwsgi/refinement-app-backend.ini
```
...
Type=simple
User=root
ExecStart=/usr/bin/uwsgi --ini /etc/uwsgi/refinement-app-backend.ini --manage-script-name --mount /=refinement:app
...
```
10. Start the backend and enable it to start automatically:
```sh
